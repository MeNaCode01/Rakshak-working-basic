...
const SOSRequestList = () => {
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      ...
      console.error(
        `Error updating SOS request ${id} with action ${action}:`,
        error
      );
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/sos/${id}`);
      setSOSRequests((prevRequests) =>
        ...
        <table className="w-full bg-gray-900 rounded-lg overflow-hidden">
          <thead className="bg-zinc-800">
            <tr className="text-left text-white">
              ...
              <th className="px-6 py-3 text-red-500 text-lg font-bold uppercase border-b border-gray-700">
                Ambulance
              </th>
              <th className="px-6 py-3 text-lg font-bold uppercase border-b border-gray-700">
                Actions
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center space-x-2">
                    <button
                      className={`py-1 px-3 rounded ${
                        request.status === "received"
                          ? "bg-green-500 hover:bg-green-600 text-white font-light"
                    ...
                    <button
                      className={`py-1 px-3 rounded ${
                        request.status === "sent"
                          ? "bg-yellow-500 hover:bg-yellow-600 text-white font-light"
                    ...
                    <button
                      className={`py-1 px-3 rounded ${
                        request.status === "pending"
                          ? "bg-red-500 hover:bg-red-600 text-white font-light"
