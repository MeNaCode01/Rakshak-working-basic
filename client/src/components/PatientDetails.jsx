...
const PatientDetails = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:3000/patients/${id}`, {
        ...formData,
        doctorNotes,
      });
      setPatient(response.data);
  ...
  return (
    <Section className="text-gray-900 flex flex-col  items-center py-10 px-4 md:px-0">
      <form onSubmit={handleSubmit} className="w-[1500px] max-w-4xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {[
            ...
            <div
              key={index}
              className={`bg-${color}-200 p-4 rounded-lg shadow-lg flex items-center`}
            >
          ...
          <div className="border-t border-b border-gray-300 py-6 mb-6">
            ...
            <div className="flex flex-wrap justify-between">
              {[
                {
                  label: "Room Temperature",
                  value: submittedData.roomTemperature,
                },
                {
                  label: "Body Temperature",
                  value: submittedData.bodyTemperature,
                },
                {
                  label: "Body Oxygen Level",
                  value: submittedData.oxygenLevel,
                },
      ...
      <div
        ref={certificateRef}
        className="bg-white p-8 rounded-lg shadow-lg max-w-4xl w-full border-4 border-purple-500 mt-8"
      >
        <div className="text-center mb-6">
          <h2 className="text-4xl font-bold text-purple-700">
        ...
        <div className="flex justify-between items-center">
          <div>
            <img
              src="/src/assets/hos-stamp.webp"
              alt="Hospital Stamp"
              className="w-52 pr-4 h-auto"
            />
          </div>
