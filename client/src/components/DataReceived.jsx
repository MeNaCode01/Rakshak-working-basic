...
const DataReceived = () => {
    ...
    const fetchData = async () => {
      if (!isLoading && contract && address) {
        try {
          ...
          setMsg([]);
          console.log("Checking received documents...");
        } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-2 p-4">
      {msg.length !== 0 ? (
        msg.map((item, index) => (
          <div key={index} className="border-transparent rounded-lg bg-zinc-600 border-white p-3 hover:bg-zinc-700 transition-colors">
            <div className="text-emerald-300 text-sm mb-1">
              <span className="font-semibold">From:</span> 
              <span className="text-white ml-2">{item.receiver ? `${item.receiver.slice(0,6)}...${item.receiver.slice(-4)}` : 'Unknown'}</span>
            </div>
            <div className="text-yellow-300 text-sm mb-1">
              <span className="font-semibold">IPFS CID:</span> 
              <span className="text-white ml-2 break-all">{item.cid || item.content}</span>
            </div>
            <div className="text-blue-300 text-sm mb-2">
              <span className="font-semibold">Received:</span> 
              <span className="text-white ml-2">{item.timestamp ? convertUTC(item.timestamp) : 'Unknown'}</span>
            </div>
        ...
        <p className="text-gray-400 text-center p-4 bg-zinc-700 rounded-lg">
          No documents received yet! When someone sends you a document, it will appear here.
        </p>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" onClick={handlePopupClose}>
          <div className="w-full max-w-4xl bg-zinc-900 rounded-lg shadow-lg overflow-hidden m-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-zinc-700">
              ...
              <button onClick={handlePopupClose} className="text-gray-400 hover:text-white focus:outline-none">
