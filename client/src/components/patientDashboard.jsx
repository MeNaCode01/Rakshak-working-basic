  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        ...
        const totalPatients =
          response.data.femalePatients +
          response.data.malePatients +
          response.data.otherPatients +
          response.data.nonPatients;
        setTotalPatients(totalPatients);

        // Calculate total appointments
        const totalAppointments =
          totalPatients + statusData.sent + statusData.pending;
          ...
          datasets: [
            {
              label: "Number of Patients",
              data: [
                ageCategories.below18,
                ageCategories.from18to40,
                ageCategories.above40,
              ],
              backgroundColor: [
