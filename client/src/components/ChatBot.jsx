  ...
  const generateAnswer = async () => {
    ...
    try {
      ...
      if (error.message === 'Request timeout') {
        setAnswer('Error: The request took too long. Please try again with a shorter query.');
        setErrorMessage('⏱️ Request timed out. The AI is taking too long to respond.');
      } else {
        ...
        <div className="right md:w-1/2 text-lg mt-4 md:mt-0 md:ml-4 rounded border h-[465px] border-zinc-800 px-8 pt-6 pb-8 mr-16 overflow-y-auto">
          ...
          {isGenerating ? (
            <div className="flex flex-col items-center justify-center h-64">
              ...
              <p className="text-purple-400 animate-pulse">Analyzing your symptoms with AI...</p>
              <p className="text-zinc-500 text-sm mt-2">This may take up to 30 seconds</p>
            </div>
          ) : answer ? (
            <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-700">
              <p className='text-zinc-300 font-light text-base leading-relaxed whitespace-pre-wrap'>{answer}</p>
            </div>
          ) : (
            <div className="flex items-center justify-center h-64 text-zinc-600">
              <p className="text-center">
                Fill out the form and click "Generate Answer" to get AI-powered medical advice.
              </p>
            </div>
          )}
        </div>
